...
void MainWindow::createConnections() {
    ...
    m_componentRemoveObserverId =
        componentRemovedObs.attachMember( this, &MainWindow::onItemRemoved );
...
void MainWindow::onSelectionChanged( const QItemSelection& /*selected*/,
    ...
    if ( m_selectionManager->hasSelection() ) {
        ...
        else { m_currentShaderBox->setCurrentText( "" ); }
    ...
    if ( ent.isRoNode() ) {
        ...
        if ( ro->getType() == Engine::Rendering::RenderObjectType::Geometry ) {
            ...
            if ( i == -1 ) {
                ...
                else { m_currentShaderBox->setItemData( i, refCount ); }
            }
        }
    }
}
...
void MainWindow::exportCurrentMesh() {
    ...
    if ( e.isRoNode() ) {
        ...
        else { LOG( logERROR ) << "Mesh from " << ro->getName() << "failed to export"; }
    }
...
void MainWindow::deleteCurrentItem() {
    ...
    else if ( e.isComponentNode() ) { e.m_entity->removeComponent( e.m_component->getName() ); }
